...
export const ToastProvider: FC<Props> = ({ children }) => {
    ...
    const updateToastList = useCallback((addToastInformation: ToastElementType) => {
...

        {
            toastList && <div className={`toast-notifications__container`}>
                {toastList.map((toast: ToastElementType) =>
                    <div className={`toast-notifications__item__${toast.type || 'default'}`} key={toast.id}>
                        <span className="toast-notifications__close"></span>
                        <h3 className={`toast-notifications__${toast.type || 'default'}__title`}>{toast.title}</h3>
                        <p className={`toast-notifications__${toast.type || 'default'}__text`}>{toast.text}</p>
                    </div>
                )}
            </div>
        }
